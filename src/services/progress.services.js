...
class ProgressService {
  ...
  async updateDailyProgress(userId, date = new Date()) {
    try {
      ...
      const totalCalories = meals.reduce((sum, meal) => 
        sum + (meal.total?.calories || 0), 0);
      
      const totalProtein = meals.reduce((sum, meal) => 
        sum + (meal.total?.protein || 0), 0);
      
      const totalCarbs = meals.reduce((sum, meal) => 
        sum + (meal.total?.carbs || 0), 0);
      
      const totalFat = meals.reduce((sum, meal) => 
        sum + (meal.total?.fat || 0), 0);
  ...
  async getWeeklySummary(userId) {
    try {
      ...
      summary.averageCalories = Math.round(summary.totalCalories / progress.length || 0);
  ...
  async getMonthlySummary(userId) {
    try {
      ...
      return {
        ...
        weight: weightByDay,
        consistency: Math.round((progress.filter(d => d.daily.mealsLogged > 0).length / 30) * 100),
